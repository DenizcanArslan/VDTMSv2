...
const columns = [
  ...
  { header: "Year", accessor: "modelYear", className: "hidden lg:table-cell" }, // Hidden on small screens
  { header: "GENSET", accessor: "genset", className: "hidden lg:table-cell" }, // Hidden on small screens
...
const TrailersListPage = async ({searchParams}) => {
  ...
  const [data, count] = await prisma.$transaction([
    ...
    prisma.trailer.count({ where: query })
  ]);
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">All Trailers</h1>
        <div className="flex flex-col md:flex-row w-full md:w-auto items-center gap-5">
          ...
          <div className="flex items-center gap-5">
            ...
            {role === "admin" && <FormModal table="trailer" type="create" />}
          </div>
        </div>
      </div>
