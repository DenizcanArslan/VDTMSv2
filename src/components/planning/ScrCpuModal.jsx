...
export default function ScrCpuModal({ isOpen, onClose, transport, onUpdate }) {
  ...
  const [scrCpuNote, setScrCpuNote] = useState(transport.scrCpuNote || '');

  useEffect(() => {
    setStatus(transport.scrCpuStatus);
    setColor(transport.scrCpuColor);
    setScrCpuNote(transport.scrCpuNote || '');
  }, [transport]);
  ...
  useEffect(() => {
    const handleTransportUpdate = (updatedTransport) => {
      if (updatedTransport.id === transport.id) {
        ...
        setScrCpuNote(updatedTransport.scrCpuNote || '');
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      onUpdate(status, color, scrCpuNote);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">SCR/CPU Note</label>
          <textarea
            className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm min-h-[60px] resize-y"
            value={scrCpuNote}
            onChange={e => setScrCpuNote(e.target.value)}
            placeholder="Enter SCR/CPU note (optional)"
          />
        </div>
